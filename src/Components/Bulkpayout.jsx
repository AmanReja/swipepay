  ...
  >
    <main className="w-full h-full flex-col overflow-y-scroll">
      <section className="w-full flex flex-col sm:flex-row gap-[20px] 2xl:h-[780px] sm:min-h-[600px] sm:h-[600px] px-[2px] sm:px-[20px]">
        ...
        >
          <form className="flex flex-col border gap-6 w-full rounded-[10px] shadow-md border-gray-200 dark:border-gray-700">
            ...
            <div className="w-full px-[20px] flex-col flex gap-[10px]">
              <div className={`flex p-4 flex-col text-[16px] text-center ${theme === "dark" ? "text-gray-300" : "text-gray-500"}`}>
              ...
              <div className="w-full px-[30px] py-[40px] flex justify-center">
                ...
                >
                  ...
                  <label className={`${theme === "dark" ? "text-gray-400" : "text-gray-400"}`} htmlFor="Preview">
                    Drag And Drop Preview
                  </label>
                </div>
              </div>
  
              <p className={`w-full text-[12px] flex items-center gap-[5px] ${theme === "dark" ? "text-blue-400" : "text-blue-500"}`}>
        ...
        >
          <div className={`flex justify-between items-center p-4 h-[60px] w-full ${
            ...
            <div className={`border-[1px] p-1 px-[20px] rounded-[8px] duration-300 ${
          ...
          <table className={`w-full text-sm text-left ${
            ...
            <thead className={`text-[11px] uppercase border-b border-t ${
              theme === "dark" ? "text-gray-400 border-gray-600 bg-gray-700" : "text-gray-400 border-gray-300 bg-[#fcfcfc]"
            }`}>
              <tr>
            ...
            <tbody className="text-[12px] font-semibold">
              {load?Array.from({length:6}).map((_,i)=>(<Contentloader key={i}/>)):bulkpayoutdata?.map((txn, i) => (
                <tr
                  key={i}
      ...
      <section className="flex justify-between gap-6 p-6 mt-[50px] rounded-xl">
        {[  {title:"Questions ?", desc:"Visit our Help Center for detailed assistance on billing, payments, and subscriptions.", img:i1, linkText:"Go to Help Center"},
            {title:"Contact Support", desc:"Need assistance? Contact our support team for prompt, personalized help your queries & concerns.", img:i2, linkText:"Contact Support"}].map((item,i)=>(
          <div key={i} className={`flex w-full flex-col border-[1px] rounded-[10px] shadow ${
            theme === "dark" ? "border-gray-700 bg-gray-800 text-gray-300" : "border-gray-100 bg-white text-gray-800"
          }`}>
            <div className="w-full flex p-6">
              <div className="flex w-[75%] flex-col p-6 gap-[5px] min-h-[180px]">
                <h1 className="text-[22px]">{item.title}</h1>
                <p className={`text-[14px] ${theme==="dark"?"text-gray-300":"text-gray-700"}`}>{item.desc}</p>
              </div>
              <div className="flex w-[25%] p-4">
                <img src={item.img} alt="" />
              </div>
            </div>
            <div className={`w-full flex border-t h-[40px] justify-center items-center ${
              theme==="dark"?"border-gray-700":"border-gray-200"
            }`}>
              <a className="text-blue-500 border-b-[1px] border-dashed text-[12px]" href="">{item.linkText}</a>
            </div>
          </div>
        ))}
      </section>
