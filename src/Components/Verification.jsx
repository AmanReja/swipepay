...
const Verification = () => {
  ...
  const handleVerificationwithnum = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      setResult({ aadhaar: "Valid", pan: "Invalid" }); 
    }, 2000);
  };
    ...
    <section className="flex flex-col w-full p-4 sm:min-h-[600px] 2xl:h-[780px]  sm:h-[600px] gap-[30px]">
      ...
      <main className="w-full p-6 sm:min-h-[600px] 2xl:h-[780px]">
        ...
        <motion.div
          ...
          className="w-full bg-white/70 backdrop-blur-xl shadow-xl rounded-2xl p-6 border border-gray-200"
        >
        ...
        <div className="flex w-full items-center justify-center gap-6 my-8 text-sm font-medium text-gray-600">
          {["Enter Details", "Upload Docs", "Verify"].map((step, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              {step}
              {i < 2 && <div className="h-px w-12 bg-gray-300"></div>}
            </div>
          ))}
        </div>
        ...
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="w-full bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-gray-200 p-8"
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <input
                ...
                maxLength="12"
                value={aadhaarNumber}
                className="mt-2 block w-full border border-gray-300 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm"
                placeholder="Enter Aadhaar Number"
              />
              <label className="mt-4 flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl p-4 text-gray-500 cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition">
                <Upload size={20} />
        ...
        {result && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="w-full max-w-4xl mt-10 bg-white/90 backdrop-blur-lg rounded-2xl shadow-lg border border-gray-200 p-6 mx-auto"
          >
